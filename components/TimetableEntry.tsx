    ...
    return (
      <div className="max-w-xl w-full bg-white rounded-[3.5rem] shadow-2xl p-16 border border-slate-100 animate-in zoom-in-95 duration-500">
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <h2 className="text-3xl font-black text-slate-900 tracking-tighter">Availability Range</h2>
          <p className="text-slate-400 text-sm mt-3 font-medium px-4">When does this schedule apply?</p>
        </div>
        <div className="space-y-6">
          ...
          <div className="pt-6 flex gap-4">
            ...
            <button onClick={() => setStep('studio')} className="flex-[2] py-5 bg-indigo-600 text-white font-black rounded-2xl shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all">Setup Activities</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-6xl mx-auto w-full h-full flex flex-col animate-in fade-in slide-in-from-bottom-12 duration-700">
      <div className="bg-white rounded-[3.5rem] shadow-2xl border border-slate-200/50 overflow-hidden flex flex-col flex-1">
        <div className="px-12 py-10 border-b border-slate-100 flex justify-between items-center shrink-0">
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tighter mb-1">Activity Setup</h2>
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">{currentUser.name}'s Items</span>
          ...
          <div className="flex items-center gap-6">
            <div className="flex flex-col items-end">
              <label className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1.5">Timezone</label>
              <select value={userTimezone} onChange={(e) => setUserTimezone(e.target.value)} className="bg-slate-50 border-none text-[10px] font-black text-indigo-600 rounded-xl px-4 py-2 ring-1 ring-slate-100 outline-none focus:ring-2 focus:ring-indigo-500/20">{commonTimezones.map((tz: string) => <option key={tz} value={tz}>{tz}</option>)}</select>
            </div>
        ...
        <div className="flex flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto p-12 custom-scrollbar bg-white">
            <form onSubmit={handleManualSubmit} className="space-y-8 max-w-2xl mx-auto">
              <div><label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-4 ml-4">Activity Name</label><input type="text" required className="w-full text-2xl font-black bg-slate-50 rounded-[1.8rem] border-none px-8 py-6 outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all placeholder:text-slate-200" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="E.g. Lunch" /></div>
              <div className="grid grid-cols-2 gap-8">
                <div><label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-4 ml-4">Day</label><select className="w-full rounded-[1.5rem] bg-slate-50 border-none px-6 py-5 font-black text-slate-700 outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all" value={day} onChange={(e) => setDay(Number(e.target.value) as DayOfWeek)}>{DAYS.map((d, i) => <option key={d} value={i}>{d}</option>)}</select></div>
                <div><label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-4 ml-4">Starts At</label><input type="time" className="w-full rounded-[1.5rem] bg-slate-50 border-none px-6 py-5 font-black text-slate-700 outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all" value={startTime} onChange={(e) => setStartTime(e.target.value)} /></div>
              </div>
              <div>
                <div className="flex justify-between items-center mb-4 px-4">
                  <label className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Duration</label>
                  <span className="text-indigo-600 font-black text-xs bg-indigo-50 px-3 py-1.5 rounded-xl">{duration} Minutes</span>
                </div>
                <input type="range" min="15" max="480" step="15" className="w-full h-2 bg-slate-100 rounded-full appearance-none cursor-pointer accent-indigo-600" value={duration} onChange={(e) => setDuration(Number(e.target.value))} />
              </div>
              <button type="submit" className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black shadow-xl hover:bg-slate-800 transition-all active:scale-[0.98]">
                {editingIndex !== null ? 'Update Draft' : 'Add to Sync Queue'}
              </button>
            </form>
          </div>

          <div className="w-[400px] bg-slate-50 overflow-y-auto p-12 shrink-0 border-l border-slate-100 flex flex-col">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8">Queue to Sync</h3>
            <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-2">
              {drafts.map((d, i) => (
                <div key={i} className="group bg-white p-6 rounded-[2rem] border border-slate-200 transition-all hover:shadow-xl">
              ...
              {drafts.length === 0 && <div className="h-40 border-2 border-dashed border-slate-200 rounded-[2.5rem] flex items-center justify-center text-[10px] font-black text-slate-300 uppercase tracking-widest text-center px-8">Queue is empty</div>}
            </div>
            {drafts.length > 0 && (
